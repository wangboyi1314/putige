...
async function getUsage(env: PagesEnv, orderId: string): Promise<number> {
  if (env.ORDERS) {
    const raw = await env.ORDERS.get(`usage:${orderId}`);
...
async function setUsage(env: PagesEnv, orderId: string, count: number): Promise<void> {
  if (env.ORDERS) {
    await env.ORDERS.put(`usage:${orderId}`, String(count), { expirationTtl: 86400 * 7 });
    return;
  }
  memUsage.set(orderId, count);
}

export async function canUsePremiumOrder(env: PagesEnv, orderId: string): Promise<boolean> {
  const usage = await getUsage(env, orderId);
  return usage < MAX_PREMIUM_CALLS_PER_ORDER;
}

export async function recordPremiumOrderUse(env: PagesEnv, orderId: string): Promise<void> {
  const usage = await getUsage(env, orderId);
